...
if control_state == in_air
  ...
  if bitget( floor( time ), 1 )
      % See Raibert paper on how psi is defined
   leg_angle_desired = body_angle + asin( ( xd*last_bounce_time/2 + kxd*(xd - speed_desired ) ) / leg_length_default);
  else
   leg_angle_desired = body_angle + asin( ( xd*last_bounce_time/2 + kxd*(xd - speed_desired ) ) / leg_length_default);
  end;
...
if control_state == on_ground_going_up
  rest_leg_length = leg_length_default + 0.0185 + leg_length_gain*(height_desired - last_max_height); %P-controller with bias to default value and 2nd feedforward term to correct for Ess
  if leg_length_new > rest_leg_length
    control_state = in_air;
    max_height = y;
    result = control_state;
